...
def compute_features(bars_1m: pd.DataFrame, p: dict) -> pd.DataFrame:
...
def generate_signals(feat: pd.DataFrame, p: dict) -> pd.DataFrame:
    ...
    out = pd.DataFrame(rows).sort_values("ts").reset_index(drop=True) if rows else \
        pd.DataFrame(columns=["ts", "signal", "trigger", "price", "imb"])
...
def run_signals(client: FMPClient, settings) -> dict:
    ...
    week_feat = compute_features(week_1m, DEFAULTS)
    base_feat = compute_features(base_1m, DEFAULTS)
    base_days = len({ts.date() for ts in base_feat.index})

    chosen, trials = tune(week_feat, base_feat, base_days)
    week_feat = compute_features(week_1m, chosen)
    base_feat = compute_features(base_1m, chosen)
    ...
    _write_report(chosen, trials, ev, week_fwd, base_fwd, base_days,
                  out_dir / "signal_validation.md")
    return {"chosen": chosen, "eval": ev, "week_signals": week_fwd,
            "baseline_signals": base_fwd}


def _write_report(chosen, trials, ev, week_fwd, base_fwd, base_days, out) -> None:
